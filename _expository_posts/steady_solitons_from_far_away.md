...
In other words, you are moving the metrics $g(t)$ by their Ricci curvature. You can obtain Ricci flow solutions given a steady solitons as follows: Let $\Phi_t$ denote the one-parameter family of [diffeomorphisms](https://link.springer.com/book/10.1007/978-1-4419-9982-5) generated by $\nabla f$ with $\Phi_0=\operatorname{id}_M$ and set $g(t):=\Phi_t^* g$. Then, $(M,g(t))_{t\in (-\infty,\infty)}$ is an eternal solution to the Ricci flow, called the \textit{canonical Ricci flow} induced by $g$. 
...
<div class="theorem" id="theorem-7">
 ([Hamilton](https://doi.org/10.1090/conm/071/954419)) Any complete two-dimensional gradient steady Ricci soliton is either flat or isometric, up to scaling, to the cigar soliton. 
...
([Bryant soliton](https://math.duke.edu/~bryant/3DRotSymRicciSolitons.pdf)) For each $n \geq 3$ there exists a complete nonflat rotationally symmetric steady soliton $(\mathbb{R}^n,\tilde g)$ called the Bryant soliton, written $\operatorname{Bry}^n$.  If $\tilde g=dr^2+w(r)^2g_{S^{n-1}}$, then 
$$
...
<div class="theorem" id="theorem-9"> ([Brendle](https://arxiv.org/abs/1202.1264)) Any complete nonflat $\kappa$-noncollapsed three-dimensional steady gradient Ricci soliton is isometric, up to scaling, to the Bryant soliton.
...
[(Brendle)](https://arxiv.org/abs/1203.0270) Any $\kappa$-noncollapsed asymptotically cylindrical steady soliton with $\sec >0$ is also isometric, up to scaling, to the Bryant soliton. 
...
<div class="theorem" id="theorem-12">
 ([Lai](https://arxiv.org/abs/2010.07272)) For every $n\ge 3$, there exists a family of $\mathbb{Z}_2\times O(n-1)$-symmetric but non-rotationally symmetric steady solitons with positive curvature operator. When $n\geq 4$, these are $\kappa$-noncollapsed. 
...
([Lai](https://arxiv.org/abs/2205.01146)) Let $(M^3,g,f)$ be any 3D steady soliton with nonnegative curvature operator that is NOT the Bryant soliton. Then, \\
- $M$ is collapsed. 
- The asymptotic limits are the Cigar soliton and $S^1\times \mathbb{R}$.
- The [asymptotic cone](https://arxiv.org/abs/2207.02714) of $M$ is a 2D sector. More precisely, if $r_k\to 0$, 
$$(M,r_kg,o)\to (X,d_X,x)$$
in the [Gromov Hausdorff sense](https://arxiv.org/abs/2012.00756) where $(X,d_X)$ is a metric space given by the sector of angle $\alpha\in (0,\pi)$ in the Euclidean plane. 
...
<div class="lemma" id="theorem-16"> ([Chan--Ma--Zhang,'23](https://arxiv.org/abs/2310.14020)) Consider any sequence $x_i\in M$ with $d(x_i,o)\to \infty$. By passing to a subsequence, $$ \left(M^4,R(x_i) g,x_i\right) \to \operatorname{Bry}^3\times \mathbb{R}\text{ or }S^2 \times R^2,$$
smoothly. 
...
Let us pass to the convergence in level sets. Suppose that $R(x_i)\to 0$. Then CMZ observed that $$\frac{f-f(x_i)}{\sqrt{R(x_i)}}\to z$$
...
For all $s\ll 0$, $\Sigma_s$ consists of [two Bryant caps and a neck joining them](https://arxiv.org/abs/1911.00091). In particular, $\#G^{-1}(0)\leq 2$. 
</div>
...
\begin{itemize}
\item[1.] $\lim _{d(x, o) \rightarrow \infty} R(x)=0$;
\item[2.] $\lim _{d(x, o) \rightarrow \infty} R(x) d(x, \Gamma)=0$;
\item[3.] the asymptotic cone of $(M, g)$ is a ray.
\end{itemize}
